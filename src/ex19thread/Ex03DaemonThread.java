...
class YesThread extends Thread {
	public YesThread() {}
	/*
	 Thread클래스의 생성자를 호출 할 때 전달하는 매개변수를
...
/*
 종속쓰레드 (Daemon Thread)
 	...
 	등의 업무를 처리한다.
 	프로그램이 종료 될 때 자동으로 종료되므로 주로 무한로프로
 	구성한다. setDaemon(true) 메소드로 설정 할 수 있다.
 */
class DaemonThread extends Thread {
	...
	public void run() {
		while(true) {
			System.out.println(
					String.format("[쓰레드명:%s]디피알위갱갱",
							getName())
			);
			try {
				sleep(3000);
				System.out.println("3초마다 갱신된.다.구?");
			}
			catch (InterruptedException e) {
				System.out.println("자동저장시 오류발생ㅠㅡㅠ");
			}
		}
	}
}
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		 ...
		 */
		NotThread nt1 = new NotThread("첫번째 클.라.쓰.랄.까?");
		nt1.notThreadMethod();
		NotThread nt2 = new NotThread("두번째 클.라.쓰.랄.까?");
		nt2.notThreadMethod();
		...
		dt.setName("난 데.몬.쓰.레.드");
		dt.setDaemon(true);//독립쓰레드를 데몬쓰레드도 만들어준다.
		dt.start();
