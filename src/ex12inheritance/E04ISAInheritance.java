...
/*
 IS-A관계
 	...
 	-노트북은 컴퓨터의 일종이다
 	-휴대전화는 전화기의 일종이다
 	이와같이 IS-A관계가 성랍하는 개체를 상속으로 표현하기에 적합하다
 */
/*
 컴퓨터를 표현
...
class Computer{
	...
	/*
	 ...
	 혹은 상속받은 클래스에서 접근이 불가능 하므로
	 public 으로 선언된 메소드를 통해서 간접 호출을 해야한다
	 상속관계라면 protected로 선언된 메소드로 호출할수있다
	 */
...
class NotebookComputer extends Computer{
	...
	/*
	 ...
	 앞에 다른 문장이 먼저오게되면 요류가 발생되므로 주의해야한다
	 */
...
/*
 테블릿 노트북 컴퓨터 
 	: 컴퓨터의 기본기능, 휴대이용애 팬으로 드로잉 할수있는 기능까지 추가된 클래스
 */
class TableNotebook extends NotebookComputer{
	...
	//테블릿 사용을 위해 배터리양 채크 및 등록된 펜인지 체크
	public void write(String penInfo) {
		...
		/*
		 A.compareTo()
		 :문자열 a와b를 비교하여 0이 반환되면 같은 문자열로
		 판단하고 -1혹은 1이반환되면 서로 다른 문자열로 판단하는String 클래스에 정의된 메소드
		 */
		if(registPencil.compareTo(penInfo)!=0) {
			System.out.println("등록된펜이 아닙니다");
			return;
		}
...
public class E04ISAInheritance {
	...
	public static void main(String[] args) {
		...
		System.out.println("==XYZ-1234 펜으로 테블릿사용==");
