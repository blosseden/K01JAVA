...
class HighFriend extends Friend{
	...
	public HighFriend(String name, String phone, String addr, String nicname) {
		super(name, phone, addr);
		this.nicname = nicname;
	}
	/*
	 고딩친구의 전체정보 출력을 위해 부모에서 정의된 메소드를
	 super를 통해 호출하고 자식에서 확장한 변수를 별도의print문으로 출력하고있다
	 */
	public void showAllDate() {
		System.out.println("==고딩친구(전체정보)==");
		super.showAllData();
		System.out.println("별명:"+nicname);
	}
	/*
	자식에서 오버라이딩 하여 재정의한 메소드로 고딩친구의 간략정보 출력
	 */
	@Override
	public void showBasicInfo() {
...
class FriendInfoHandler{
	/*
	 맴버변수
	 Friend 타입의 객체배열은 하위클래스인 High, Univ객체를 
	 모두 저장할수 있으므로 하나의 배열내에 2개의 객체를 동시에 저장하여 관리할수있다
	 */
	...
	/*
	 ...
	 2.High객체가 Friend 타입으로 자동형변환되어 저장
	 3.객체배열에 저장된 객체들을 for문을 통해 순차적으로 출력
	 이때 오버라이딩된 메소드를 통해 항상 해당 객체의 정보를 출력할수있음
	 (참조변수의 타입에 영향을 받지않음) ★주소출력
	 */
	...
	public void searchInfo() {
		...
		for(int i=0; i<numOfFriends;i++) {
			...
			//검색할 이름과 객체의 이름이 일차하는 경우 모든정보를 출력함
...
public class E10MyFriendInfoBook {
	...
	public static void main(String[] args) {
		//프로그램 전반에 대한 비즈니스로직을 가진 핸들러 객체생성
		...
		while(true) {
			...
			case 7:
				...
				return; //메인함수의 종료는 프로그램 종료로 이어진다
			}
		}
	}
}
