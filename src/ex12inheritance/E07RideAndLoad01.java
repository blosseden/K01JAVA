...
/*
 ...
 오버라이딩 : 함수명, 개매변수, 반환형까지 모두 동일한 함수를
 상속관계에 있는 하위클래스에서 재정의 한것을 말한다
 이경우 하위클래스에서 정의한 메소드가 상위클래스의 메소드를
 가리게 되므로 항상 최하위 클래스의 메소드가 호출되게 된다
...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 즉 아래부분은 모두 C클래스에 생성된 메소드가 호출된다
		 */
		System.out.println("오버라이딩 처리된 메소드");
		ref1.rideMethod();
		ref2.rideMethod(); //3개 모두 C에 정의된 메소드 호출
		ref3.rideMethod();
		...
		ref1.loadMethod(); //A클래스 영영까지 접근가능
		...
		/*
		 ...
		 참조변수로 참조할 수없다 자식타입의 참조변수로
		 부모객체를 참조하는것은 불가능하다
		 */
		//B refId2 = refId1;
		//C refId3 = refId1;
		//아래와 같이 다운캐스팅(강제형변환)하면 참조 가능
		C refId3=(C)refId1;
	}
