...
/*
 오버라이딩(Overriding)
 	...
 	멤버메소드와 완전한 모양으로 재정의하는것을 말한다
 	오버로딩과 다른점은 매개변수의 갯수, 타입, 반환형이
 	완전히 똑같은 형태로 정의하는것이다.
 	하위클래스에서 오버라이딩한 메소드는 상위클래스의 메소드를 
 	가리게 되므로 항상 최하위 클래스에서 오버라이딩한 메소드가 호출하게된다
 */
//일반적인 스피커를 추상화한 클래스
class Speaker{
...
class BaseSpeaker extends Speaker{
	...
	/*
	 @Override : '어노테이션' 이라고 부르고 오버라이딩 된 
	 			메소드에 붙어 컴파일러에게 알려주는 역할
	 */
	@Override
	public void showState() {
		//super를 통해 부모클래스에 정의된 멤버메소드를 호출
		super.showState();
	...
	public static void main(String[] args) {
		...
		*/
		...
		/*
		 ...
		 메소드는 호출할수 없다 setBase 는 자식클래스에 정의된 멤버 메소드이기 떄문
		 단, 오버라이딩 처리된 메소드가 있는경우 하위클래스의 메소드가 호출되게 된다
		 */
