...
public class E02SilsuType {
	...
	public static void main(String[] args) {
		...
		/*
		 float과 long의 연산결과를 롱으로 받고 싶다면
		 방법1 : float을 long으로 형변환후 연산하거나
		 방법2 : 계산 결과 전체를 long으로 명시적(강제)형변환 한다.
		 */
		long result2 = long1 + (long)float1;
		System.out.println("형변환후 연산^q^="+ result2);
		long result3 = (long)(long1 + float1);
		System.out.println("계산후 형변환^오^="+ result3);
		
		/*
		 - 실수형에서 기본 형은 double이다.
		 - 소수점이 있는 데이터를 컴파일러는 무조건 double로 인식한다.
		 - 단, 소수점이 없는 데이터는 float에 대입 할 수 있다
		...
		float f4 = (float)3.14; //형변환후 대입(권장사항 아님)
