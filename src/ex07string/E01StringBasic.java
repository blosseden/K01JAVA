	...
	public static void main(String[] args) {
		...
		/*
		 String 클래스의 객체생성 방법1 : new를 사용한다.
		 	...
		 	항상 새로운 메모리를 ,할당한다.
		 */
		String str1 = new String("안녕자바야");
		String str2 = new String("안녕자바야");
		/*
		 str1, str2는 String객체의 참조값을 가지고 있으므로
		 아래는 참조값에 대한 비교를 진행한다.
		 */
		if(str1==str2) {
			System.out.println("str1과 str2는 참조주소가 같다");
		}
		else {
			System.out.println("다름 암튼 다름");
		}
		/*
		 equals() 메소드
		 	: Object클래스로 부터 상속받은 메소드로 실제 저장된
		 	문자열을 비교하도록 만들어진 메소드이다.
		 	즉는 객체에 실제 저장된 문자열에 대한 비교가
		 	이루어진다.
		 */
		if(str1.equals(str2)) {
		 ...
		 */
		if(str3==str4) {
			System.out.println("주소가 같따");
		}
		else {
			System.out.println("달랑");
		}
