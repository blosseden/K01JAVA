...
public class E02StringMethod {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 191919-4444444
		 */
		String juminNum = "970326-1111111";
		if(juminNum.charAt(7)=='1'
			...
			System.out.println("여자");
					...
					|| juminNum.charAt(7)=='6') {
			System.out.println("외국놈");
		}
		else {
			System.out.println("주민번호잘못치심");
		}
		
		/* 4] compareTo() : 두문자열을 첫번째 문자부터 순차적으로
				비교하면서 앞의 문자열의 아스키코드가 크면 양수를 반환
		...
		System.out.println("JAVA"+" SORLD"+" GO");
		...
		/*
		 ...
		 프로그램을 작성하시오.
		 krst736@naver.com => 이멜형식
		 nosdjfaksafj;ad=> 아님
		 */
		String email1 = "krst7336@navercom";
		if(email1.contains("@") && email1.contains(".")) {
			System.out.println("이멜 형식임");
		}
		else {
			System.out.println("이멜형식 아님");
		}
		...
		/*
		 7] endsWith()
		 	:문자열이 특정 문자열로 끝나면 true, 아니면
		 	 false를 반환한다.
		 8] startsWith()
		 	: 문자열이 특정 문자열로 시작하며 true, 아니면
		 	false를 반환한다.
		 */
		System.out.println("www.doum.net".endsWith("net"));
		...
		/*
		 12] replace
		 	: 특정문자열을 찾아서 지정된 문자열로 변경한다.
		 	만약 찾는 문자열이 없다면 병경만 되지 않을뿐 에러가
		 	발생하지는 않는다.
		 */
